...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('import_df_form_comp_value_usos.log', encoding='utf-8')
    ]
)
logger = logging.getLogger("importador_usos")
...
def main():
    logger.info("Iniciando proceso de importación de df_form_comp_value para gerencia USOS...")
    ...
    # 3. Obtener transacciones pendientes para Usos (basado en gedos egresados de la vista)
    try:
        logger.info("Buscando transacciones asociadas a expedientes de usos (trx_gedo, trx_ee, trx_tad)...")
        query_pendientes = """
            SELECT DISTINCT id_transaction 
            FROM (
                SELECT trx_gedo AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE id_expediente IN (SELECT id_expediente FROM public.mv_usos_gedos_egreso) AND trx_gedo IS NOT NULL
                UNION
                SELECT trx_ee AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE id_expediente IN (SELECT id_expediente FROM public.mv_usos_gedos_egreso) AND trx_ee IS NOT NULL
                UNION
                SELECT trx_tad AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE id_expediente IN (SELECT id_expediente FROM public.mv_usos_gedos_egreso) AND trx_tad IS NOT NULL
            ) t
            WHERE NOT EXISTS (
        ...
        logger.info(f"Se encontraron {len(pendientes)} transacciones pendientes de importación para USOS.")
    except Exception as e:
