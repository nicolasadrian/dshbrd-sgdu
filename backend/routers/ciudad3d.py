...
@router.get("/api/ciudad3d/analistas_trazados")
def list_analistas_trazados(current_user: User = Depends(get_current_user)):
    try:
        with engine.connect() as conn:
            rows = conn.execute(text("""
                SELECT username, full_name, role, permissions
            ...
            for r in rows:
                u_name = r[0]
                f_name = r[1] or u_name
                role = (r[2] or "").lower()
                perms = r[3] or {}
                
                # Filtrar usuarios que tienen permiso de dibujar LFI, revisar LFI o rol trazados/admin
                if perms.get("lfi_dibujar") or perms.get("lfi_revisar") or role in ['admin', 'administrador', 'trazados', 'trazador']:
                    analistas.append({
                        "username": u_name,
                        "full_name": f_name
                    })
        ...
        logger.error(f"Error listing analistas trazados: {e}")
        raise HTTPException(status_code=500, detail=str(e))
