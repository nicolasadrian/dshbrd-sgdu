...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('import_df_form_comp_value_ifmad.log', encoding='utf-8')
    ]
)
logger = logging.getLogger("importador_ifmad")
...
def main():
    logger.info("Iniciando proceso de importación de df_form_comp_value para acrónimo IFMAD...")
    ...
    # 3. Obtener transacciones pendientes para IFMAD
    try:
        logger.info("Buscando transacciones asociadas a documentos IFMAD en 2022, 2023, 2024, 2025 y 2026...")
        query_pendientes = """
            SELECT DISTINCT id_transaction 
            FROM (
                SELECT trx_gedo AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE acronimo = 'IFMAD' AND EXTRACT(YEAR FROM fecha_asociacion) IN (2022, 2023, 2024, 2025, 2026) AND trx_gedo IS NOT NULL
                UNION
                SELECT trx_ee AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE acronimo = 'IFMAD' AND EXTRACT(YEAR FROM fecha_asociacion) IN (2022, 2023, 2024, 2025, 2026) AND trx_ee IS NOT NULL
                UNION
                SELECT trx_tad AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE acronimo = 'IFMAD' AND EXTRACT(YEAR FROM fecha_asociacion) IN (2022, 2023, 2024, 2025, 2026) AND trx_tad IS NOT NULL
            ) t
            WHERE NOT EXISTS (
        ...
        logger.info(f"Se encontraron {len(pendientes)} transacciones pendientes de importación para IFMAD.")
    except Exception as e:
