...
def run_populate(target_name, db_url):
    print(f"\n=======================================================")
    print(f"POPULATING SLA TABLES FOR: {target_name}")
    print(f"=======================================================")
    ...
    print(f"Finished {target_name} in {round(time.time() - t0, 2)}s!")

if __name__ == '__main__':
    loc_url = os.getenv("DATABASE_URL_LOCAL", "postgresql://postgres:lenovo@localhost:5432/sade_db")
    run_populate("LOCAL SADE_DB", loc_url)
    
    prod_url = os.getenv("DATABASE_URL_PUBLIC") or os.getenv("DATABASE_URL")
    if prod_url:
        run_populate("PROD SADE_DB", prod_url)
