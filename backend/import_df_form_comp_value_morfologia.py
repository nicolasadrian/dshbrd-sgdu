...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('import_df_form_comp_value_morfologia.log', encoding='utf-8')
    ]
)
logger = logging.getLogger("importador_morfologia")
...
def main():
    logger.info("Iniciando proceso de importación de df_form_comp_value para gerencia MORFOLOGIA...")
    ...
    # 3. Obtener transacciones pendientes para Morfología (basado en gedos egresados de la vista)
    try:
        logger.info("Buscando transacciones asociadas a expedientes de morfología (trx_gedo, trx_ee, trx_tad)...")
        query_pendientes = """
            SELECT DISTINCT id_transaction 
            FROM (
                SELECT trx_gedo AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE id_expediente IN (SELECT id_expediente FROM public.mv_morfologia_gedos_egreso) AND trx_gedo IS NOT NULL
                UNION
                SELECT trx_ee AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE id_expediente IN (SELECT id_expediente FROM public.mv_morfologia_gedos_egreso) AND trx_ee IS NOT NULL
                UNION
                SELECT trx_tad AS id_transaction FROM public.mvw_datos_gedo_secgdu 
                WHERE id_expediente IN (SELECT id_expediente FROM public.mv_morfologia_gedos_egreso) AND trx_tad IS NOT NULL
            ) t
            WHERE NOT EXISTS (
        ...
        logger.info(f"Se encontraron {len(pendientes)} transacciones pendientes de importación para MORFOLOGIA.")
    except Exception as e:
    ...
    for i in range(total_lotes):
        ...
        try:
            ...
            # Insertar en Postgres en la tabla compartida df_form_comp_value
            if filas_oracle:
                insert_query = """
            ...
            # Marcar como importados en la tabla de control compartida
            control_query = """
