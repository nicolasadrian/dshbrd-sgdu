-- ============================================================
-- ARCHIVO 99: Queries de validación de Morfología
-- ============================================================
-- Ejecutar DESPUÉS de crear todas las MVs.
-- ============================================================

-- ===========================================
-- 1) Conteo del universo
-- ===========================================
SELECT 
    es_trata_propia,
    COUNT(*) AS cant
...
-- ===========================================
-- 2) Conteo general por categoría (trámites propios)
-- ===========================================
SELECT
    (SELECT COUNT(*) FROM mv_morfologia_stock_propio)            AS stock_propio,
    (SELECT COUNT(*) FROM mv_morfologia_subsanaciones)           AS subsanaciones,
...
-- ===========================================
-- 3) Coherencia: ¿las categorías son disjuntas?
-- ===========================================
SELECT 
    (SELECT COUNT(*) FROM mv_morfologia_universo WHERE es_trata_propia)  AS total_universo_propio,
    COUNT(DISTINCT id_expediente)                                        AS expedientes_unicos_clasificados
FROM (
    SELECT id_expediente FROM mv_morfologia_stock_propio
    UNION ALL
    SELECT id_expediente FROM mv_morfologia_subsanaciones
    UNION ALL
    SELECT id_expediente FROM mv_morfologia_egresos_efectivos
    UNION ALL
    SELECT id_expediente FROM mv_morfologia_egresos_no_efectivos
) t;
...
-- ===========================================
-- 4) Conteo de intervenciones
-- ===========================================
SELECT
    (SELECT COUNT(*) FROM mv_morfologia_universo WHERE NOT es_trata_propia)   AS universo_intervenciones,
    (SELECT COUNT(*) FROM mv_morfologia_intervenciones_stock)                  AS interv_stock,
    (SELECT COUNT(*) FROM mv_morfologia_intervenciones_subs)                   AS interv_subs,
    (SELECT COUNT(*) FROM mv_morfologia_intervenciones_egresadas)              AS interv_egresadas,
    (SELECT COUNT(*) FROM mv_morfologia_interv_egresos_eventos)                AS eventos_egreso_interv;


-- ===========================================
-- 5) Reporte mensual de TRÁMITES PROPIOS (mayo 2026)
-- ===========================================
WITH 
...
ingresos_mes AS (
    ...
    FROM mv_morfologia_ingresos_eventos, parametros p
    WHERE es_trata_propia = TRUE
...
egresos_efec_mes AS (
    SELECT trata, COUNT(*) AS cant
    FROM mv_morfologia_gedos_egreso, parametros p
    WHERE fecha_egreso >= p.desde AND fecha_egreso < p.hasta
    GROUP BY trata
),
egresos_noef_mes AS (
    SELECT trata, COUNT(*) AS cant
    FROM mv_morfologia_egresos_no_efectivos, parametros p
    WHERE fecha_ultimo_movimiento >= p.desde AND fecha_ultimo_movimiento < p.hasta
    GROUP BY trata
),
stock_actual AS (
    SELECT trata, COUNT(*) AS cant FROM mv_morfologia_stock_propio GROUP BY trata
),
subs_actual AS (
    SELECT trata, COUNT(*) AS cant FROM mv_morfologia_subsanaciones GROUP BY trata
)
SELECT
...
-- ===========================================
-- 6) Stock histórico por mes (totales)
-- ===========================================
SELECT 
    mes_label,
