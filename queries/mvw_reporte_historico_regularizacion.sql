...
ingresos_raw AS (
    ...
    WHERE p.destinatario IN ('DGROC-OBRASDEMO', 'DGROC-ESPERAINSTALACIONES')
    GROUP BY 1, 2, 3
),
ingresos AS (
...
egresos_efectivos AS (
    ...
    SELECT p.id_expediente, 'INTERVENCIONES' as trata, 'regularizacion' as gerencia, MIN(p.fecha)::date as fecha_egr
    FROM pases_pre_filtrados p
    JOIN ingresos i ON p.id_expediente = i.id_expediente
    WHERE i.trata = 'INTERVENCIONES' AND p.fecha > i.fecha_ing
      AND p.remitente IN ('AGUEROJO', 'AKRACOFF', 'ALVAREZ.M', 'ARAOZLUIS', 'ATENCIOAL', 'DALBORAF', 'ENCISOA', 'EPARLATO', 'ERDOCIAINA', 'JBARRACO', 'JLGARMENDIA', 'JTERRILE', 'MYUSHU', 'S.SANCHEZPAZ', 'SCAVALLARO')
      AND p.destinatario NOT IN ('AGUEROJO', 'AKRACOFF', 'ALVAREZ.M', 'ARAOZLUIS', 'ATENCIOAL', 'DALBORAF', 'DGROC-ESPERAINSTALACIONES', 'DGROC-OBRASDEMO', 'ENCISOA', 'EPARLATO', 'ERDOCIAINA', 'JBARRACO', 'JLGARMENDIA', 'JTERRILE', 'MYUSHU', 'S.SANCHEZPAZ', 'SCAVALLARO')
    GROUP BY 1, 2, 3
),
...
SELECT 
    ...
    ) IN ('AGUEROJO', 'AKRACOFF', 'ALVAREZ.M', 'ARAOZLUIS', 'ATENCIOAL', 'DALBORAF', 'DGROC-ESPERAINSTALACIONES', 'DGROC-OBRASDEMO', 'ENCISOA', 'EPARLATO', 'ERDOCIAINA', 'JBARRACO', 'JLGARMENDIA', 'JTERRILE', 'MYUSHU', 'S.SANCHEZPAZ', 'SCAVALLARO')) as "STOCK_PROPIO",
