-- Reporte Histórico CATASTRO (ESPEJO LOCAL - MVW)
...
subs_mensuales AS (
    SELECT 
        l.trata_reporte, p.anio, p.mes,
        COUNT(DISTINCT pases.id_expediente) as stock_subs
    FROM (
        SELECT id_expediente, fecha as fecha_inicio, LEAD(fecha) OVER (PARTITION BY id_expediente ORDER BY fecha) as fecha_fin, estado
        FROM mvw_ee_pases_secgdu
    ) pases
    JOIN lifecycle l ON pases.id_expediente = l.id_expediente
    CROSS JOIN periodos p
    WHERE pases.estado ILIKE 'Subsanaci%' 
      AND pases.fecha_inicio <= p.fin_mes 
      AND (pases.fecha_fin IS NULL OR pases.fecha_fin > p.fin_mes)
    GROUP BY 1, 2, 3
)
SELECT 
