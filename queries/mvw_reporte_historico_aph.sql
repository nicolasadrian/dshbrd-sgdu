-- Vista Materializada: APH
DROP MATERIALIZED VIEW IF EXISTS mvw_reporte_historico_aph;
...
WITH tramites_metadata (gerencia, trata, nombre_trata, acronimos_list) AS (
    VALUES 
    ('aph', 'MDUG3701A', 'Consulta Obligatoria para Inmuebles en APH o Catalogados.', ARRAY['DI', 'ANEXO', 'IF']),
    ('aph', 'MDUG3801A', 'Solicitudes Especiales para Inmuebles en APH o Catalogados.', ARRAY['DI', 'ANEXO', 'IF']),
    ('aph', 'INTERVENCIONES', 'Intervenciones', ARRAY['DI', 'ANEXO', 'IF'])
),
periodos AS (
    SELECT 
...
ingresos_raw AS (
    -- Ingresos para trámites puros (Buzón DGIUR-21)
    SELECT p.id_expediente, 'aph' as gerencia_buzon, ec.trata as trata_orig, MIN(p.fecha)::date as fecha_ing
    FROM pases_pre_filtrados p
    JOIN expedientes_target ec ON p.id_expediente = ec.id_expediente
    WHERE p.destinatario = 'DGIUR-21' 
      AND ec.trata IN ('MDUG3701A', 'MDUG3801A')
    GROUP BY 1, 2, 3
    
    UNION ALL
    
    -- Ingresos para Intervenciones (Buzón DGIUR-21, trata no pura)
    SELECT p.id_expediente, 'aph' as gerencia_buzon, ec.trata as trata_orig, MIN(p.fecha)::date as fecha_ing
    FROM pases_pre_filtrados p
    JOIN expedientes_target ec ON p.id_expediente = ec.id_expediente
    WHERE p.destinatario = 'DGIUR-21'
      AND ec.trata NOT IN ('MDUG3701A', 'MDUG3801A')
    GROUP BY 1, 2, 3
),
ingresos AS (
...
egresos_efectivos AS (
    ...
    WHERE i.trata = 'INTERVENCIONES'
      ...
      AND p.remitente IN ('CHANTIRRO', 'CHEZOM', 'DAMATOG', 'DESANTISA', 'GALAMA', 'GONZALEZNIETOR', 'HERENUFE', 'LSANTINMOLINA', 'MARIANALVAREZ', 'NASALVATIERRA', 'PIOLON', 'VASTAM')
      AND p.destinatario NOT IN ('CHANTIRRO', 'CHEZOM', 'DAMATOG', 'DESANTISA', 'GALAMA', 'GONZALEZNIETOR', 'HERENUFE', 'LSANTINMOLINA', 'MARIANALVAREZ', 'NASALVATIERRA', 'PIOLON', 'VASTAM', 'DGIUR-21', 'DGIUR-ADMISIBILIDADAPH', 'DGIUR-ADMISIMIDIDADAPH', 'SVC_DGIURADMAPH')
    GROUP BY 1, 2, 3
),
...
SELECT 
    ...
    ) IN ('CHANTIRRO', 'CHEZOM', 'DAMATOG', 'DESANTISA', 'DGIUR-21', 'DGIUR-ADMISIBILIDADAPH', 'DGIUR-ADMISIMIDIDADAPH', 'GALAMA', 'GONZALEZNIETOR', 'HERENUFE', 'LSANTINMOLINA', 'MARIANALVAREZ', 'NASALVATIERRA', 'PIOLON', 'SVC_DGIURADMAPH', 'VASTAM')) as "STOCK_PROPIO",
